        ...
        <q-card-section class="q-gutter-y-sm">
          ...
          <q-scroll-area style="height: 260px">
            <purchase-row
              ...
              :price="`R$ ${formatCurrency(p.attributes.price)}`"
              :isShared="p.attributes.isShared"
              :isDividing="isDividing"
              :purchasers="{ data: p.attributes.purchasers.data }"
              :tags="{ data: p.attributes.tags.data }"
              :blured="isSelecting"
              :isCurrentlySelected="selectedPurchaseId == p.id"
              :displayTaggingCheckbox="isTagging"
              :isTagged="taggedPurchases.includes(p.id)"
              @click="handlePurchaseSelection(p.id)"
              @dividePurchaseClick="isDividing = true"
              @tagCheckboxToggle="(id: number) => handleTagCheckboxToggle(id)"
              @tagRemove="
                (tag) => {
                  handleTagRemove(
